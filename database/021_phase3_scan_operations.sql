...
ALTER TABLE `gate_scan_events`
  ADD COLUMN `processing_started_at` datetime DEFAULT NULL AFTER `claimed_at`,
  ADD COLUMN `completed_at` datetime DEFAULT NULL AFTER `processing_started_at`;

CREATE INDEX `idx_scan_processing_started`
  ON `gate_scan_events` (`result`, `processing_started_at`);
