...
// Hardcoded allowlist: only additive/idempotent migrations belong here.
// Never replace this with a glob because database/ also contains
// reference/destructive SQL files.
...
function appliedMigrationsSafe(PDO $pdo): array
{
    if (!tableExists($pdo, 'schema_migrations')) return [];
...
foreach ($tenants as $tenant) {
    ...
    if (!$dryRun) ensureMigrationsTable($tenantDb);
    $applied = appliedMigrationsSafe($tenantDb);
    $ranAny = false;
    ...
    foreach (INCREMENTAL_MIGRATIONS as $migration) {
        ...
        try {
            ...
            $tenantDb->prepare('INSERT INTO schema_migrations (migration) VALUES (:m)')->execute([':m'=>$migration]);
    ...
    if (!$ranAny && !$dryRun) echo "  (nothing to do)\n";
    echo "\n";
}
