...
final class VisitService
{
    ...
    {
        // FIX: was $dto->$dto->visitor_id — removed erroneous self-dereference
    ...
    public function checkIn(int $visitId): void
    {
        ...
        if ((int) $visitor['is_blacklisted'] === 1) {
            throw new RuntimeException('Blacklisted visitors cannot check in.');
        }
    ...
    public function checkOut(int $visitId): void
    {
        ...
        if ($this->badgeRepo->hasActiveBadge($visitId)) {
            throw new RuntimeException('Badge must be returned before checkout.');
        }
    ...
    // ── QUERIES — tenantId params removed ────────────────────

    // FIX: was `int int $visitorId`
    public function getVisitor(int $visitorId): ?array
    {
        return $this->visitorRepo->find($visitorId);
    }

    // FIX: removed int $tenantId param
    public function activeVisits(): array
    {
        return $this->visitRepo->getActiveVisits();
    }

    // FIX: removed int $tenantId param
    public function getDepartments(): array
    {
        return $this->visitRepo->getDepartments();
    }

    // FIX: removed int $tenantId param
    public function getHosts(): array
    {
        return $this->visitRepo->getHosts();
    }

    // FIX: removed int $tenantId param
    public function getVisitTypes(): array
    {
        return $this->visitRepo->getVisitTypes();
    }
}
