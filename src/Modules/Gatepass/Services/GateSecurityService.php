...
/**
 * Phase 3 gate security boundary.
 *
 * Device credentials are presented only as a secret and are never
 * persisted in plaintext. QR credentials are likewise opaque random
 * values; only their SHA-256 digests are stored.
 */
final class GateSecurityService
{
    ...
    /**
     * Authenticate an approved gate device and its current assignment.
     * Returns the assignment/device context or null when authentication fails.
     */
    public function authenticateDevice(string $deviceUuid, string $deviceSecret, int $gateId, ?int $guardUserId = null): ?array
    {
        ...
        if ($deviceUuid === '' || $deviceSecret === '' || $gateId < 1) {
            return null;
        }
        ...
        $expected = (string) $device['device_secret_hash'];
        $presentedHash = hash('sha256', $deviceSecret);

        // Constant-time comparison prevents a timing side channel.
        if (!hash_equals($expected, $presentedHash)) {
            return null;
        }

        if ($guardUserId !== null && $device['guard_user_id'] !== null
            && (int) $device['guard_user_id'] !== $guardUserId) {
            return null;
        }

        $this->db->prepare(
            'UPDATE approved_devices SET last_seen_at = NOW() WHERE id = :id AND is_active = 1 AND revoked_at IS NULL'
        )->execute([':id' => (int) $device['device_id']]);

        return $device;
    }

    /** Generate a one-time device secret for provisioning. */
    ...
    {
        if ($secret === '') {
            throw new RuntimeException('Device secret cannot be empty.');
        }

        return hash('sha256', $secret);
    }

    /** Generate an opaque QR credential. Never use the gatepass DB id. */
    ...
    {
        if ($token === '') {
            throw new RuntimeException('QR token cannot be empty.');
        }
    ...
    /**
     * Validate an opaque QR credential. Returns the active gatepass or null.
     */
    public function resolveQrToken(string $token): ?array
    {
        ...
        $stmt = $this->db->prepare("
            SELECT g.*
            FROM gatepasses g
            WHERE g.qr_token_hash = :hash
              AND g.deleted_at IS NULL
              AND g.qr_revoked_at IS NULL
              AND (g.qr_expires_at IS NULL OR g.qr_expires_at > NOW())
            LIMIT 1
        ");
